...
/* --- VERSIÓN MODIFICADA PARA LA ANIMACIÓN --- */
void py_timestep_leapfrog(struct py_field *f, double Nt)
{
    char frame_filename[STR_SIZE];
    // Directorio de salida. Asegúrate de que exista.
    const char* output_dir = "Linux_x86_64/output";

    // Guardamos el estado inicial (frame 0) antes de empezar el bucle
    sprintf(frame_filename, "%s/frame_%04d.tsv", output_dir, 0);
    py_write_to_disk(f->p, frame_filename);

    for (long n = 1; n <= Nt; ++n) { // El bucle ahora va de 1 a Nt
        py_leapfrog(f); // Un paso de la simulación

        // Guardar un frame cada 20 pasos de tiempo.
        // Ajusta el '20' para tener más o menos frames. Un valor más bajo = animación más suave pero más lenta de generar.
        if (n % 20 == 0) {
            // Crear un nombre de archivo único para cada frame.
            // Usamos 'n' para nombrar el frame.
            sprintf(frame_filename, "%s/frame_%04ld.tsv", output_dir, n);
            
            // Usamos la función que ya existe para escribir en disco el campo de presión.
            py_write_to_disk(f->p, frame_filename);
        }
    }
}
